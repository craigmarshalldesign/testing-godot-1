...
func update_target_display() -> void:
	...
	var current_actor: Node = CombatManager.current_combatant
	if current_actor and not current_actor.is_in_group("player"):
		target = current_actor
	
	# Priority 2: If it's player's turn, show the player's selected target
	if not target and player.state is CombatPlayerState:
		var combat_state := player.state as CombatPlayerState
		target = combat_state.current_target
...
func _on_end_turn_pressed() -> void:
	...
	if player_ref and player_ref.state is CombatPlayerState:
		var state_ref: CombatPlayerState = player_ref.state as CombatPlayerState
		state_ref.end_player_turn(player_ref)
