...
func start_combat(player: Node, initial_enemy: Node) -> void:
	...
	for enemy: Node in all_enemies:
		if enemy == initial_enemy:
			continue
		if enemy.global_position.distance_to(fight_center) < 20.0:
	...
	# Tell everyone to enter combat mode
	for c: Node in combatants:
		if c.has_method("enter_combat"):
			c.enter_combat()
...
func end_combat() -> void:
	is_combat_active = false
	for c: Node in combatants:
		if is_instance_valid(c) and c.has_method("exit_combat"):
			c.exit_combat()
	combatants.clear()
